...
head_to_head_L <- list()
for(i in 1:30){
  head_to_head_L[[i]] <- list()
  for(j in 1:30){
    head_to_head_L[[i]][[j]] <- list()
  }
}
...
for(i in 1:1231){
  ...
  head_to_head_L[[idx_1]][[idx_2]][[m_mat[idx_2, idx_1]]] <- match_i_1
  head_to_head_L[[idx_2]][[idx_1]][[m_mat[idx_1, idx_2]]] <- match_i_2
}
